...
//  ██ CUSTOMER MANAGEMENT ██
// ══════════════════════════════════════════════════════════════════
import { custState } from '../../shared/state.js';
import { escHtml, findColumnKey, triggerDownload } from '../../shared/utils.js';
import { agentBridge } from '../../shared/agent-client.js';

export async function custLoadCustomers() {
...
function _custRefreshOrDropdowns() {
  ...
  const inGroups = new Set(custState.orGroups.flat());
...
function custAddOrGroup() {
  ...
  custState.orGroups.push([left, right]);
...
function custRemoveOrGroup(idx) {
  custState.orGroups.splice(idx, 1);
  _custRefreshOrGroupList();
  _custRefreshOrDropdowns();
  _custRefreshRuleSummary();
}
...
function _custRefreshOrGroupList() {
  ...
  if (custState.orGroups.length === 0) {
    list.innerHTML = '';
    return;
  }
  list.innerHTML = custState.orGroups.map((g, i) =>
...
function _custRefreshRuleSummary() {
  ...
  if (custState.docMode === 'all') { summary.style.display = 'none'; return; }
...
function custGetDocRules() {
  ...
  if (custState.docMode === 'all') return [];

  const checked = _custGetCheckedDocs();
  const inOrGroups = new Set(custState.orGroups.flat());
  ...
  const orRules = custState.orGroups
...
window.custLoadCustomers = custLoadCustomers;
window.custRenderTable = custRenderTable;
window.custOpenModal = custOpenModal;
window.custCloseModal = custCloseModal;
window.custEdit = custEdit;
window.custSave = custSave;
window.custDelete = custDelete;
window.custReactivate = custReactivate;
...
window.custOpenImportModal = custOpenImportModal;
