...
const INV_CSV_ALIASES = {
  ...
  containerNumber: ['containernumber', 'container', 'containerid', 'containerno', 'cntr', 'cntrnumber', 'cntrno'],
...
// ── Invoice Sender Log ──
function invAddLog(level, msg) {
  const log = document.getElementById('invStatusLog');
  const t = new Date().toLocaleTimeString('en-US', { hour12: false });
  log.innerHTML += `<div class="log-${level}"><span class="log-time">[${t}]</span> ${escHtml(msg)}</div>`;
  log.scrollTop = log.scrollHeight;
  // Auto-expand the log when an error occurs so the user can see what went wrong
  if (level === 'error' && invoiceState.logCollapsed) invToggleLog();
}

function invClearLog() {
  document.getElementById('invStatusLog').innerHTML = '';
}

function invToggleLog() {
  const body = document.getElementById('invStatusLogBody');
  const arrow = document.getElementById('invLogToggleArrow');
  invoiceState.logCollapsed = !invoiceState.logCollapsed;
  if (invoiceState.logCollapsed) {
...
// ── Invoice Sender Progress ──
function invSetProgress(pct, label) {
  const bar = document.getElementById('invProgressBar');
  const lbl = document.getElementById('invProgressLabel');
  const con = document.getElementById('invProgressContainer');
  if (pct === null) {
...
function invSaveSendStatus(invoiceNumber, status, extra) {
  try {
    const history = JSON.parse(localStorage.getItem('ngl_send_history') || '{}');
    ...
    localStorage.setItem('ngl_send_history', JSON.stringify(history));
...
function invRestoreSendHistory() {
  try {
    ...
    if (cleaned) localStorage.setItem('ngl_send_history', JSON.stringify(history));
...
function invSaveLastRunSummary(summary) {
  try {
    localStorage.setItem('ngl_last_send_run', JSON.stringify({
...
async function invSendViaQBO() {
  ...
  // Stream events
  sendJobEventSource = agentBridge.streamProgress(result.jobId, invHandleSendEvent);
}
...
function invShowInlineApproval(event) {
  ...
  section.innerHTML = `
    ...
    <div style="background:#fff; border-radius:8px; padding:12px 14px; margin-bottom:12px; font-size:0.8rem; line-height:1.7; border:1px solid ${borderDetail};">
      ...
      <div><strong>Attachments:</strong> ${(event.attachmentsFound || []).length ? escHtml(event.attachmentsFound.join(', ')) : '<span style="color:#d97706;">None detected</span>'}</div>
...
async function invApprovalDecision(approve) {
  ...
  if (approve) {
    ...
    const res = await agentBridge.approveSend(sendState.jobId);
...
async function invPauseSendJob() {
  if (!sendState.jobId) return;
  try {
    await fetch(agentBridge.baseUrl + '/jobs/' + sendState.jobId + '/pause', { method: 'POST' });
